...
it('lets look up token id via star name', async () => {
    ...
    await instance.createStar("awesome star look up", starId, { from: user1 });
    ...
    assert.equal(starLookUp, "awesome star look up");
});
...
it('can add the star name and star symbol properly', async () => {
    ...
    assert.equal(contractName, "Decentralized Start Token");
    assert.equal(contractSymbol, "DST");
});
